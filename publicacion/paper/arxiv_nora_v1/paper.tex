...
NASA's Open Science Data Repository (OSDR; \citealt{OSDR2024}) consolidates astronaut-derived omics under a common release schema, including the SpaceX Inspiration4 commercial mission \citep{Sanders2024i4} ($n=4$, 3-day low-Earth-orbit, 9 OSD studies, used here). External Earth-baseline analogues---Antarctic winter-over confinement \citep{Kuipers2020antarctic}, 6-month ISS HRV \citep{Edgell2019iss}, parabolic-flight autonomic response \citep{Schlegel1998parabolic}---are essential to anchor multi-year extrapolation, and shrinkage-pooled hierarchical models naturally incorporate such priors.

\subsection{Autonomous research agents}
Recent structured multi-agent AI systems \citep{Zhou2026nora,Lu2026ai,Cheng2026mind2report} demonstrate that a domain-specialised arrangement of modular procedures, specialised sub-tasks and lifecycle controls substantially improves the reliability and reproducibility of research workflows compared with general-purpose LLM use. The methodology pipeline reported here was developed under an instance of this pattern (Section~\ref{sec:harness}). To our knowledge this is the first public application of a structured, generator--evaluator AI workflow to the analysis of NASA HRP datasets.
...
Posterior inference uses the No-U-Turn Sampler \citep{Hoffman2014nuts} specified in PyMC \citep{Salvatier2016pymc} with the NumPyro/JAX backend \citep{Phan2019numpyro}: two chains at target acceptance $0.9$, drawing 400 posterior samples after 400 warm-up iterations for each LOSO fold, and 600 after 600 for the full-data coefficient fit. Convergence is monitored by $\hat{R}$, bulk and tail effective sample sizes (ESS), and the number of divergent transitions, which are persisted to \texttt{publicacion/\_mcmc\_diagnostics.json}; non-convergent fits are rejected and re-run with increased warm-up.
...
\subsection{AI-assisted, human-supervised workflow}\label{sec:harness}

The methodology pipeline was developed under an AI-assisted, human-supervised workflow following the generator--evaluator pattern formalised by \citet{Zhou2026nora}. The workflow comprises five elements:

\begin{enumerate}
\item \textbf{Modular workflow logic}. Domain-specialised procedure modules---for academic-rigour rewriting, \LaTeX{} layout, scientific-computing scaffolding and life-support/ECLSS calculation---encode the workflow logic outside the language model as human-readable specifications, so that each step is auditable independently of any particular model.
\item \textbf{Generator--evaluator separation}. A generator step drafts code and prose under author direction; an independent evaluator step then red-teams every artefact against project-specific rubrics before it is accepted. The same adversarial separation is applied to the present manuscript: claims that did not survive the critical pass were revised or removed.
...
\item \textbf{State persistence}. A structured provenance log captures pipeline position, last-completed step, accepted artefacts and recovery hints, so the workflow resumes deterministically from the exact interruption point and never re-runs an already-accepted stage.
\item \textbf{Safety gates}. Visual review of every figure is mandatory before a deliverable is declared complete (validation-by-metadata is insufficient); every released artefact is paired with a dated backup, and the evaluator red-team is invoked after any change to a deliverable.
\end{enumerate}

This arrangement is itself a contribution: it documents in reproducible detail \emph{how} a single independent researcher can produce a methodology pipeline of journal-grade rigour in a workflow that conventionally requires a multi-disciplinary team. The boundary of AI assistance is explicit: scientific assumptions, model choices and parameter calibration are owned by the author; the language-model assistance is confined to drafting, code skeletons and validation cross-checks. No automated component held authority over any scientific conclusion. Section~\ref{sec:author-contributions} carries the formal CRediT statement.

\subsection{Software, seeds and reproducibility}

All analyses are implemented in Python 3.11 with: \texttt{numpy} 2.2.6, \texttt{scipy} 1.16.2, \texttt{pandas} 2.3.3, \texttt{scikit-learn} 1.7.2, \texttt{pymc} 5.28.5, \texttt{numpyro} 0.21.0 \citep{Phan2019numpyro}, \texttt{jax} 0.10.0, \texttt{arviz} 0.23.4, \texttt{matplotlib} 3.10.6. A fixed master seed (42) is set across all stochastic components, and posterior sampling is fully determined by that seed through the PyMC/NumPyro interface. The complete pinned environment is captured in \texttt{requirements.txt} and the run-time provenance log in \texttt{publicacion/\_demo\_report\_2026-05-09.html}. All experiments executed on a single consumer workstation (x86-64, 16~GB RAM) without GPU, with total wall-clock under 15 minutes.
...
\subsection{Classifier sensitivity}\label{sec:results-classifier}

To test whether the per-modality accuracies of Section~\ref{sec:methods-loso} are an artefact of the distance-based baseline, we repeated the LOSO protocol under identical folds, preprocessing and Beta--Binomial uncertainty quantification with three classifiers: $k$-nearest neighbours ($k=5$), a random forest (500 trees), and a support-vector machine with an RBF kernel. Hyper-parameters were fixed a priori; no per-fold tuning was performed, to avoid optimistic bias at $n=4$. Table~\ref{tab:clf} reports the results.

\begin{table}[t]
\centering
\small
\caption{LOSO accuracy by classifier and modality (identical folds and preprocessing). Distance-based kNN understates the high-dimensional modalities that a tree ensemble recovers; the most-informative modality is itself classifier-dependent (\textbf{bold} = best per row).}
\label{tab:clf}
\begin{tabular}{@{}lrrr@{}}
\toprule
Modality & kNN & Random Forest & SVM-RBF \\
\midrule
Urine cytokine (Alamar) & \textbf{0.688} & 0.542 & 0.625 \\
Cardiovascular (EVE)    & \textbf{0.679} & 0.607 & \textbf{0.679} \\
Serum cytokine (Alamar) & 0.625 & \textbf{0.804} & 0.667 \\
MMASH proxy             & 0.636 & \textbf{0.773} & 0.545 \\
Immune EVE              & 0.571 & \textbf{0.714} & 0.643 \\
Blood CBC               & 0.571 & \textbf{0.714} & 0.571 \\
Metabolic CMP           & \textbf{0.571} & \textbf{0.571} & 0.536 \\
Microbiome pathway      & 0.406 & \textbf{0.561} & 0.449 \\
Microbiome taxonomy     & 0.404 & \textbf{0.652} & 0.442 \\
\bottomrule
\end{tabular}
\end{table}
...
\begin{figure}[t]
\centering
\includegraphics[width=0.92\linewidth]{../../figures_publicacion/fig_E_classifier_comparison.png}
\caption{Per-modality LOSO accuracy under three classifiers. The random forest lifts the two microbiome modalities from chance ($0.40$--$0.41$ under kNN) to $0.56$--$0.65$, and the serum panel from $0.625$ to $0.804$, whereas the urine-cytokine result is highest under kNN. The horizontal line marks the $0.50$ chance level.}
\label{fig:E}
\end{figure}

Under a random forest, the two microbiome modalities rise from chance ($0.404$/$0.406$ under kNN) to $0.652$ and $0.561$, the serum panel from $0.625$ to $0.804$, and both the immune-EVE and blood-CBC panels from $0.571$ to $0.714$. The distance-based kNN therefore \emph{understates} the information content of the highest-dimensional readouts, where Euclidean neighbours degrade under $p \gg n$ while trees select informative axes. Critically, the identity of the single most-informative modality is itself classifier-dependent: urine cytokines lead under kNN, whereas serum cytokines lead under the random forest. We consequently frame all modality-informativeness statements as conditional on the estimator (Section~\ref{sec:discussion}, \textbf{C1$'$}), and present this comparison as a sensitivity analysis rather than as evidence of a specific effect size, which remains unresolved at $n=4$.

\subsection{Significance: per-feature testing versus multi-modal permutation}\label{sec:results-significance}

Two complementary tests bound what is statistically defensible at $n=4$. First, a per-feature Welch $t$-test of post- versus pre-flight values across all 1112 aligned features yields 77 nominal hits at $p<0.05$ --- indistinguishable from the $\approx 53$ expected by chance --- and \textbf{zero features survive Benjamini--Hochberg correction} at $q<0.05$ (Figure~\ref{fig:I}). Conventional per-feature inference thus detects nothing in this $p \gg n$ regime, which is precisely the motivation for the shrinkage and fusion approach taken here.

Second, we test the fused signal directly with a label-permutation null: a PCA-reduced logistic classifier under leave-one-subject-out achieves accuracy $0.708$ on the true pre/post labels, against a null distribution (2000 permutations) centred at chance ($0.489 \pm 0.107$), giving an empirical $p = 0.026$ (Figure~\ref{fig:J}). The multi-modal representation therefore carries a pre/post-flight signal that is \emph{statistically significant even though no individual biomarker is} --- a signature recoverable only by fusing modalities. This is the central positive result of the study and it is reported with its $n=4$ caveat intact: the effect is significant within the fixed analysis pipeline, not a calibrated population effect size.

\begin{figure}[t]
\centering
\includegraphics[width=0.92\linewidth]{../../figures_publicacion/fig_I_perfeature_fdr.png}
\caption{Per-feature post-vs-pre testing across 1112 aligned features. (Left) the p-value histogram is approximately uniform, as expected under a global null. (Right) volcano plot; the number of features surviving Benjamini--Hochberg FDR at $q<0.05$ is zero, so no single biomarker is individually significant at $n=4$.}
\label{fig:I}
\end{figure}

\begin{figure}[t]
\centering
\includegraphics[width=0.82\linewidth]{../../figures_publicacion/fig_J_permutation_null.png}
\caption{Label-permutation test for the fused multi-modal representation. The observed leave-one-subject-out logistic accuracy (0.708, red) lies in the upper tail of the permuted-label null (grey; 2000 permutations, mean 0.489), giving empirical $p=0.026$. Multi-modal fusion recovers a spaceflight signature that per-feature testing cannot.}
\label{fig:J}
\end{figure}
...
\textbf{C1.} Among non-invasive surrogates, urine cytokines are the most informative under the distance-based baseline (LOSO accuracy 0.688, 95\% CrI {[}0.47, 0.84{]}). For Artemis~II this supports prioritising frequent urine-panel collection over the more invasive blood metabolic profile at comparable information yield.

\textbf{C1$'$ (classifier caveat).} The ranking of the most-informative modality is conditional on the estimator (Section~\ref{sec:results-classifier}): under a random forest the serum cytokine panel is strongest (0.804) and both microbiome modalities move credibly above chance. We therefore do not claim any modality is uninformative; we claim only that a distance-based classifier is the wrong tool in this $p \gg n$ regime, and that operational prioritisation should be validated against the estimator intended for the live Artemis~II analysis.

\textbf{C2.} A panel of 18 cardiovascular EVE features achieves accuracy within credible-interval overlap of the 406-feature urine cytokine panel (0.679 vs.\ 0.688), and this compactness is robust across classifiers (kNN and SVM both 0.679). The feature-efficiency frontier (Figure~\ref{fig:C}) supports compact instrumentation: fewer features, sufficient information.
...
\textbf{C5 (central result).} Multi-modal fusion recovers a statistically significant pre/post-flight signature ($p=0.026$ by permutation; Section~\ref{sec:results-significance}) in a regime where no individual biomarker survives multiple-testing correction. The methodological implication for Artemis~II is concrete: at $n=4$, the return on a single-marker screen is negligible, whereas jointly modelling a compact multi-modal panel is both statistically defensible and operationally efficient. This is a positive, reproducible finding delivered under fully quantified small-$n$ uncertainty.
...
\item The headline per-modality table uses a kNN baseline (interpretability + minimal hyper-parameters). We now report a random-forest and SVM-RBF comparison (Section~\ref{sec:results-classifier}, Table~\ref{tab:clf}), which shows the modality ranking to be estimator-dependent; a fully tuned, nested-CV classifier study and gradient-boosted trees remain future work.
...
All datasets are public and listed in Table~\ref{tab:data}; identifiers resolve to NASA OSDR (\url{osdr.nasa.gov}), PhysioNet (\url{physionet.org}) and CDC NCHS (\url{cdc.gov/nchs/nhanes}). The reproducible code base, paper source, harmonised master table and provenance log are released open-source under the MIT licence at \url{https://github.com/engimine/HRP_Artemis_II}, following the close of the NASA Artemis~II HRP Methodology Challenge submission window on \mbox{5~June~2026}. A persistent archival snapshot is deposited on Zenodo (DOI: \href{https://doi.org/10.5281/zenodo.21015202}{10.5281/zenodo.21015202}). Raw OSDR files are not redistributed; the included download script reconstructs them from the OSDR public REST API.
